...
def showData(nClasses, nItemsInClass):
    ...
    classColormap = ListedColormap(['#FF0000', '#00FF00', '#FF1FFF'])
...
def classifyKNN(trainData, testData, k, numberOfClasses):
    ...
    for testPoint in testData:
        # Calculate distances between test point and all the train points
        testDist = [[dist(testPoint, trainData[i][0]), trainData[i][1]] for i in range(len(trainData))]
        # How many points of each class among nearest K
        stat = [0 for i in range(numberOfClasses)]
        for d in sorted(testDist)[0:k]:
            stat[d[1]] += 1
        # Assign a class with the largest number of occurrences among K nearest neighbours
...
def calculateAccuracy(nClasses, nItemsInClass, k, testPercent):
    ...
    print("Accuracy: ",
          sum([int(testDataLabels[i] == testDataWithLabels[i][1])
               for i in range(len(testDataWithLabels))]) / float(len(testDataWithLabels)))
